...
def load_image_robust(image_path):
    ...
    # Try tifffile first as it works better
    try:
    ...
    # If both fail, try PIL directly as last resort
    try:
...
class ImageMatcher(tk.Tk):
    def __init__(self, dir1=None, dir2=None, dir3=None):
        ...
        # Directories to load images from
        self.directories = [
        ...
        # Store image paths and thumbnails
        self.image_paths = [[], [], []]
        self.thumbnails = [[], [], []]
        self.selected_indices = [-1, -1, -1]
        self.matches = []  # List of tuples (path1, path2, path3)
        ...
        # At the end of __init__ method, after loading all directories
        # Update the main progress bar to show total remaining matches
    ...
    def create_ui(self):
        ...
        # Create three columns for image galleries
        self.gallery_frames = []
        ...
        for i in range(3):
            ...
            # Create a frame with scrollbar for listbox
            ...
            # Create a listbox for image selection with reduced height
            ...
            # Create a label for image preview with increased size
        ...
        # Status label showing current number of matches
    ...
    def load_directory(self, directory_index, directory):
        ...
        # Get image paths without duplicates
        image_paths = []
        extensions = ['.tif', '.tiff', '.jpg', '.jpeg', '.png']
        
        # Use a set to track seen paths in a case-insensitive way
        seen_paths = set()
        
        # Scan directory just once for all files
        for file_path in Path(directory).iterdir():
            if file_path.is_file():
                # Check if extension matches any in our list (case-insensitive)
                for ext in extensions:
                    if file_path.name.lower().endswith(ext):
                        # Check if we've seen this file before (case-insensitive)
        ...
        for i, path in enumerate(tqdm(image_paths, desc=f"Loading dir {directory_index+1}")):
            try:
                ...
                # Create thumbnail using robust loading
                img = load_image_robust(path)
                img.thumbnail((200, 200))  # Larger thumbnails
    ...
    def create_match(self):
        # Check if images are selected in all columns
        ...
        for i in range(3):
            if self.selected_indices[i] >= 0:
                # Remove from image_paths and thumbnails
